...
```
"Note: A newer version of Diataxis-Documentation skill is available ($INSTALLED_VERSION → $LATEST_VERSION).
...
if [ -f "./.prd/.diataxis.md" ]; then
  INIT_STATUS=$(grep -E "^init_status:" ./.prd/.diataxis.md | sed 's/^init_status: //')
  if [ "$INIT_STATUS" = "complete" ]; then
    ...
    echo ""
    echo "=== Saved Answers ==="
    grep -E "^(sources|purpose|roles|diataxis_|hosting|technology):" ./.prd/.diataxis.md || echo "(none)"
  else
...
**After EACH question is answered, immediately save to `.prd/.diataxis.md`:**

```bash
# Create/update config file with current progress
cat > ./.prd/.diataxis.md << 'EOF'
...
- Q2 → `purpose:`
- Q3 → `roles:`
- Q4 → `diataxis_<role>:` (one per role)
- Q5 → `hosting:`
- Q6 → `technology:`
...
```bash
mkdir -p .prd
cat > ./.prd/.diataxis.md << 'SOURCES_EOF'
init_status: in_progress
sources:
...
SOURCES=$(sed -n '/^sources:/,/^[a-z_]*:/{ /^sources:/d; /^[a-z_]*:/d; p; }' ./.prd/.diataxis.md)
```
...
- If `HAS_CODE=true` → recommend "Within a project"
- If `HAS_CODE=false` → recommend "Standalone docs repo"

**After answer, save immediately:**
```bash
echo "context: [within_project | standalone_docs]" >> ./.prd/.diataxis.md
```
...
Sources ✓ (Phase 1)
    ↓
Context ✓ (Phase 2 - within project vs standalone)
    ↓
...
echo "purpose: [selected purpose]" >> ./.prd/.diataxis.md
```
...
echo "roles: [selected roles]" >> ./.prd/.diataxis.md
```
...
echo "diataxis_developers: ✓,-,✓,-" >> ./.prd/.diataxis.md

# Example: User selects "How-to Guides" and "Explanation" for users:
echo "diataxis_users: -,✓,-,✓" >> ./.prd/.diataxis.md

# Example: User selects all four for operators:
echo "diataxis_operators: ✓,✓,✓,✓" >> ./.prd/.diataxis.md
```
...
**After answer, save immediately:**
```bash
echo "hosting: [selected hosting]" >> ./.prd/.diataxis.md
```
...
**After answer, save immediately:**
```bash
echo "technology: [selected technology]" >> ./.prd/.diataxis.md
```
...
```bash
CONTEXT=$(grep -E "^context:" ./.prd/.diataxis.md | sed 's/^context: //')
TECHNOLOGY=$(grep -E "^technology:" ./.prd/.diataxis.md | sed 's/^technology: //')
...
SOURCES=$(sed -n '/^sources:/,/^[a-z_]*:/{/^sources:/d; /^[a-z_]*:/d; p;}' ./.prd/.diataxis.md)
CONTEXT=$(grep -E "^context:" ./.prd/.diataxis.md | sed 's/^context: //')
PURPOSE=$(grep -E "^purpose:" ./.prd/.diataxis.md | sed 's/^purpose: //')
ROLES=$(grep -E "^roles:" ./.prd/.diataxis.md | sed 's/^roles: //')
HOSTING=$(grep -E "^hosting:" ./.prd/.diataxis.md | sed 's/^hosting: //')
TECHNOLOGY=$(grep -E "^technology:" ./.prd/.diataxis.md | sed 's/^technology: //')
```

**Rewrite `.prd/.diataxis.md` as final config (with init_status: complete):**
...
DIATAXIS_DEVELOPERS=$(grep -E "^diataxis_developers:" ./.prd/.diataxis.md 2>/dev/null | sed 's/^diataxis_developers: //' || echo "")
DIATAXIS_USERS=$(grep -E "^diataxis_users:" ./.prd/.diataxis.md 2>/dev/null | sed 's/^diataxis_users: //' || echo "")
DIATAXIS_OPERATORS=$(grep -E "^diataxis_operators:" ./.prd/.diataxis.md 2>/dev/null | sed 's/^diataxis_operators: //' || echo "")
DIATAXIS_CONTRIBUTORS=$(grep -E "^diataxis_contributors:" ./.prd/.diataxis.md 2>/dev/null | sed 's/^diataxis_contributors: //' || echo "")
...
echo "✓ Configuration finalized: ./.prd/.diataxis.md"
```
...
echo "$ROLES" | tr ',' '\n' | while read role; do
  ...
  PRIORITIES=$(grep -E "^${DIATAXIS_KEY}:" ./.prd/.diataxis.md | sed "s/^${DIATAXIS_KEY}: //")
...
Created:
- .prd/.diataxis.md (project configuration)
