...
❓ OPEN_CHOICES: [one-line list of enumerable unresolved choices, or 'none']
...
**OPEN_CHOICES rule:** any non-empty value on this line MUST be paired with an `AskUserQuestion` invocation in the same turn. The value `none` is always valid — use it only when no enumerable multi-option decisions remain unresolved (2–4 discrete options; not free-text or subjective). See `PAI/AISTEERINGRULES.md` "AskUserQuestion for choices" for the full trigger list.
